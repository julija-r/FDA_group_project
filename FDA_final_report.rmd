...
ggplot() +
  ...
  labs(
    title = "Smoothed Electricity Consumption per Person",
    x = "Month",
    y = "kWh/person"
  ) +
  theme_minimal() +
...
Summary of function-on-function regression model:
\begin{itemize}
...
\begin{itemize}
\item  We have grouped the Municipalities into two clusters: Cluster 2 showing higher electricity consumption in winter months and lower in summer while Cluster 1 shows a significant rise in consumption in summer season.
\item  Cluster is the most influential predictor of the energy consumption levels, followed by wage in both function-on-scalar and function-on-function regression models.
\item Energy consumption is most predictable during summer (when tourism, agriculture, and construction activities reach stable, peak operating patterns) and winter. 
\item Predicting energy consumption during spring and autumn is more challenging. 
\item Exploring new potential predictors could help improve predictability.
\end{itemize}
