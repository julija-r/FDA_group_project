...
Wages, count of commercial customers, and, surprisingly, unemployment have a positive coefficient, indicating that higher values of these predictors are related to higher consumption levels. Belonging to Cluster 1 also has a positive effect on consumption. Meanwhile, count of residential customers has negative coefficients, suggesting an opposite effect.

The intercept captures baseline seasonal consumption patterns. The count of commercial customers coefficients show distinct seasonal patterns with peaks at the beginning of and the middle of the year, indicating that this regressor is more influential during those periods. This could be related to seasonal business cycle. Other independent variables show some variation throughout the year, except for he count of residential customers, which is almost constant year-round.
...
plot(eval_points, commercial_importance_fosr, type="l", lwd=2, col="black",
     ...
     ylim=c(0, max(commercial_importance_fosr, residential_importance_fosr, 
                  unemployment_importance_fosr, wages_importance_fosr, cluster_importance_fosr) * 1.2),
     xaxt="n")
...
ylim2 <- c(min(0, min(commercial_coef), min(residential_coef), min(unemployment_coef), min(unemployment_vel_coef), min(cluster_coef)), 
           max(commercial_coef, residential_coef, R2_by_time, unemployment_coef, unemployment_vel_coef, wages_coef, cluster_coef))
...
legend("topleft", 
       legend=c("Commercial effect", "Residential effect", "Unemployment effect", "Unemployment velocity effect", "Wages effect", "Cluster effect"), 
...
The graph visualizing how $R^2$ varies across the months suggests that energy consumption is most predictable during summer when tourism, agriculture, and construction activities reach stable, peak operating patterns. In contrast, predicting energy consumption during colder months is more challenging because
heating demand introduces weather-dependent variability. This suggests, that additional predictors (e.g., weather temperature) could potentially help to better predict energy consumption during those months.

The plot showing confidence intervals for all coefficient functions reveals that only wages and cluster effect is consistently statistically significant throughout the year as its confidence intervals do not include zero at any point. Other predictors have a non-uniform significance throughout the year.
...
\item Model performance fluctuates significantly across seasons
\item Best predictability: summer and winter months ($R^2 \approx 0.65$)
\item Spring and autumn months are most challenging to predict ($R^2 \approx 0.1$)
