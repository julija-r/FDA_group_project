...
ggplot() +
  geom_point(data = consumption_by_population_2024, aes(x = Month, y = kWh_per_person, color = Savivaldybe), alpha = 0.5) +
...
lt_map <- st_read("municipalities.geojson")
...
Additionally, to examine seasonal variations in electricity consumption, three maps were created: winter average consumption, summer average consumption, and the seasonal difference between winter and summer.

```{r}
# seasonal averages
winter_summer_data <- consumption_by_population_2024 %>%
  ...
  group_by(Savivaldybe, season) %>%
  summarize(avg_consumption = mean(kWh_per_person, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = season, values_from = avg_consumption) %>%
  mutate(seasonal_difference = Winter - Summer,
         seasonal_ratio = Winter / Summer)

map_seasonal <- lt_map %>%
  left_join(winter_summer_data, by = c("name" = "Savivaldybe"))

# global limits for consumption values
consumption_min <- min(c(map_seasonal$Winter, map_seasonal$Summer), na.rm = TRUE)
consumption_max <- max(c(map_seasonal$Winter, map_seasonal$Summer), na.rm = TRUE)

# global limits for difference values
diff_min <- min(map_seasonal$seasonal_difference, na.rm = TRUE)
diff_max <- max(map_seasonal$seasonal_difference, na.rm = TRUE)

# Winter consumption
winter_map <- ggplot(map_seasonal) +
  geom_sf(aes(fill = Winter), color = "white", size = 0.2) +
  scale_fill_viridis_c(option = "inferno", 
                      na.value = "lightgrey",
                      name = "kWh/person",
                      limits = c(consumption_min, consumption_max)) +
  labs(title = "Winter Electricity Consumption",
       subtitle = "Average for December, January, February") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14),
...
# summer consumption map
summer_map <- ggplot(map_seasonal) +
  geom_sf(aes(fill = Summer), color = "white", size = 0.2) +
  scale_fill_viridis_c(option = "inferno",  
                      na.value = "lightgrey",
                      name = "kWh/person",
                      limits = c(consumption_min, consumption_max)) +
  labs(title = "Summer Electricity Consumption",
       subtitle = "Average for June, July, August") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14),
...
# seasonal difference map 
diff_map <- ggplot(map_seasonal) +
  geom_sf(aes(fill = seasonal_difference), color = "white", size = 0.2) +
  scale_fill_viridis_c(option = "plasma", 
                      na.value = "lightgrey",
                      name = "Winter - Summer\n(kWh/person)",
                      limits = c(diff_min, diff_max)) +
  labs(title = "Seasonal Variation in Electricity Consumption",
       subtitle = "Difference Between Winter and Summer Averages") +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14),
...
```{r}
# municipalities with biggest seasonal differences
top_diff_municipalities <- map_seasonal %>%
  st_set_geometry(NULL) %>%
  dplyr::select(name, seasonal_difference) %>%
  filter(!is.na(seasonal_difference)) %>%
  arrange(desc(seasonal_difference)) %>%
  slice_head(n = 3)

# municipalities with lowest seasonal differences
bottom_diff_municipalities <- map_seasonal %>%
  st_set_geometry(NULL) %>%
  dplyr::select(name, seasonal_difference) %>%
  filter(!is.na(seasonal_difference)) %>%
  arrange(seasonal_difference) %>%
  slice_head(n = 3)

cat("Municipalities with highest seasonal differences (Winter - Summer):\n")
print(top_diff_municipalities)

cat("\nMunicipalities with lowest seasonal differences (Winter - Summer):\n")
print(bottom_diff_municipalities)
```

Municipalities with:

- Largest increase of consumption in Winter: Elektrėnai, Vilnius district, Anykščiai
- Largest decrease of consumption in Winter: Pakruojis, Palanga, Neringa.

#### Clustering K = 2

To identify distinct electricity consumption patters accross Lithuanian municiaplities, we employed performed k-means clustering with the AkjBk model, selecting $K=2$ based on Bayesian Information Criterion (BIC) optimization. The regularization parameter $\lambda$ was set to 0.

```{r}
library(funFEM)

# Clustering
res_electricity <- funFEM(fd_obj_all_mun, K = 2, model = "AkjBk", init = "kmeans", lambda = 0, disp = TRUE)
...
- Model 1: intercept + wages
- Model 2: intercept + wages + commercial customers per person
- Model 3: intercept + wages + commercial + residential customers per person
- Model 4: intercept + wages + commercial + residential customers per person + unemployment
...
Next, we performed fnction-on-function regression, where not only the dependent variable but also the predictors were functions.
...
All predictors have an optimal $\lambda$ value of $10^{-5}$, indicating that minimal smoothing is preferred. The optimal number of basis functions are as follows: for intercept 24, commercial and residential customer counts 20, unemployment 13, unemployment rate 17.
...
The graph visualizing how $R^2$ varies across the months suggests that summer-early autumn patterns are more predictable from the included variables than consumption in other months.
